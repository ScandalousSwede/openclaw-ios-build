...
The helper then rechecks the registration and current pairing before sending.
...
- `not_sent_unavailable`: input, pairing, registration, configuration, or the
  final checks prevented a send. A thrown `authorizeSend` callback also returns this.
