...
<AccordionGroup>
  ...
  <Accordion title="Session control">
    ...
    - `sessions.describe` returns one gateway session row for an exact session key. Set the optional boolean `metadataOnly: true` for operational discovery without decoding stored `skillsSnapshot` or `systemPromptReport` payloads or reading transcript enrichment. This mode suppresses derived titles, last-message previews, transcript usage/model fallbacks, and plugin discovery/normalization even when preview options are also true. Already-active provider policy may still supply model metadata. Session visibility rules still apply. Omitted or false preserves ordinary reads. Other stored row metadata may contain user-authored text; this option is not a general content redactor.
