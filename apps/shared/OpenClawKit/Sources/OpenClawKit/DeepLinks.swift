...
public struct GatewayConnectDeepLink: Codable, Sendable, Equatable {
        ...
        fallbackEndpoints: [GatewayConnectEndpoint] = [])
    {
        ...
        self.fallbackEndpoints = Self.validatedFallbackEndpoints(
            primary: .init(host: self.host, port: port, tls: tls),
            candidates: fallbackEndpoints)
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        let tls = try container.decode(Bool.self, forKey: .tls)
        ...
        self.init(
            ...
            port: port,
            tls: tls,
            bootstrapToken: try container.decodeIfPresent(String.self, forKey: .bootstrapToken),
            token: try container.decodeIfPresent(String.self, forKey: .token),
            password: try container.decodeIfPresent(String.self, forKey: .password),
            fallbackEndpoints: fallbacks)
        guard self.isValidEndpoint else {
            throw DecodingError.dataCorruptedError(
                forKey: .host,
                in: container,
                debugDescription: "Gateway endpoint is invalid or insecure")
        }
    }
    ...
    public func selectingEndpoint(_ endpoint: GatewayConnectEndpoint) -> GatewayConnectDeepLink {
        guard endpoint.isValidEndpoint else { return self }
        return .init(
            host: endpoint.host,
            port: endpoint.port,
            tls: endpoint.tls,
    ...
    private static func decodeSetupPayload(from data: Data) -> GatewayConnectDeepLink? {
        ...
        let remainingCandidateBudget = max(0, self.maximumSetupEndpoints - urlCandidates.count)
        urlCandidates.append(contentsOf: (payload.urls ?? []).prefix(remainingCandidateBudget))
        var seenEndpoints = Set<String>()
        var links: [GatewayConnectDeepLink] = []
        for rawURL in urlCandidates {
            guard links.count < self.maximumSetupEndpoints else { break }
            let url = rawURL.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !url.isEmpty,
                  let link = self.fromGatewayURLString(
                url,
                bootstrapToken: payload.bootstrapToken,
                token: payload.token,
                password: payload.password)
            else { continue }
            let key = GatewayConnectEndpoint.endpointKey(host: link.host, port: link.port, tls: link.tls)
    ...
    private static func validatedFallbackEndpoints(
        primary: GatewayConnectEndpoint,
        candidates: [GatewayConnectEndpoint]) -> [GatewayConnectEndpoint]
    {
        var seen = Set([primary.endpointKey])
        var accepted: [GatewayConnectEndpoint] = []
        for candidate in candidates.prefix(self.maximumSetupEndpoints - 1) {
            guard accepted.count < self.maximumSetupEndpoints - 1 else { break }
            guard candidate.isValidEndpoint, seen.insert(candidate.endpointKey).inserted else { continue }
...
public struct GatewayConnectEndpoint: Codable, Sendable, Equatable {
    ...
    public var isValidEndpoint: Bool {
        guard (1...65535).contains(self.port), !self.host.isEmpty else { return false }
        var components = URLComponents()
        components.scheme = self.tls ? "wss" : "ws"
        components.host = self.host
        components.port = self.port
        guard components.url?.host != nil else { return false }
        return self.tls || LoopbackHost.isLocalNetworkHost(self.host)
    }

    fileprivate var endpointKey: String {
        Self.endpointKey(host: self.host, port: self.port, tls: self.tls)
    }

    fileprivate static func endpointKey(host: String, port: Int, tls: Bool) -> String {
        "\(tls ? "wss" : "ws")|\(host.lowercased())|\(port)"
    }
}
