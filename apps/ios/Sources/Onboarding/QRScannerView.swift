...
final class QRScannerResultHandoff {
    /// SwiftUI's onDismiss can precede VisionKit's AV capture teardown. Delay pairing UI
    /// briefly so fingerprint probing and trust presentation cannot race camera shutdown.
...
struct QRScannerView: UIViewControllerRepresentable {
    ...
    final class QRScannerContainerViewController: UIViewController {
        ...
        override func viewDidAppear(_ animated: Bool) {
            ...
            // VisionKit owns the camera session. Starting only after UIKit finishes
            // presentation keeps capture startup out of the sheet-construction phase.
    ...
    final class Coordinator: NSObject, DataScannerViewControllerDelegate {
        ...
        private func deliver(_ result: QRScannerResult, scanner: DataScannerViewController) {
            self.handled = true
            // VisionKit has no teardown-completion callback. Stop capture before asking the
            // owner to dismiss; the owner delivers this typed result only after onDismiss.
