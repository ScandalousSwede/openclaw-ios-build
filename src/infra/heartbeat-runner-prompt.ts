...
import { selectAgentSystemEvents, selectAdmittedSystemEvents } from "./system-event-ownership.js";
import {
...
export async function resolveHeartbeatPreflight(params: {
  ...
  const pendingEventEntries = (
    await selectAdmittedSystemEvents(
      selectAgentSystemEvents(peekSystemEventEntries(session.sessionKey), params.agentId),
    )
