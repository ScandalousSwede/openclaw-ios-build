...
describe("process-local scheduled sender authorization", () => {
  it.each(["cron-event", "heartbeat", "exec-event"])(
    "resolves explicit owners for %s without adding a delivery route",
    (provider) => {
      const ctx = { ...context(), Provider: provider };
      ...
      expect(JSON.stringify({ Provider: ctx.Provider, From: ctx.From, To: ctx.To })).toBe(before);
    },
  );
  ...
  it.each(["Provider", "From", "To", "SenderId", "SenderE164", "AccountId"] as const)(
    "rejects a binding after identity field %s changes",
    (field) => {
      const ctx = context();
      bindScheduledSenderChannel(ctx, "discord");
      ctx[field] = "changed";
  ...
  it("cannot attach scheduled authorization to an unrelated provider", () => {
    const ctx = { ...context(), Provider: "untrusted-event" };
    bindScheduledSenderChannel(ctx, "discord");
    expect(owner(ctx)).toBe(false);
  });
  it("preserves ordinary direct-message ownership", () => {
    const ctx = { ...context(), Provider: "discord", Surface: "discord", SenderId: "123" };
