...
type ArtifactReviewDecision = "accept_artifact" | "reject_artifact";
type ArtifactReviewCommand = Readonly<{
  binding: ArtifactReviewBinding;
  idempotency_key: string;
}>;
type ArtifactReviewResolution = ArtifactReviewCommand &
  Readonly<{
    id: string;
...
export function parseArtifactReviewBinding(value: unknown): ArtifactReviewBinding {
  ...
  return Object.freeze({
    ...
    artifact_sha256: Object.freeze(hashes.toSorted()),
  });
}
export function artifactReviewActorFromClient(client: Client): ArtifactReviewActor {
  ...
  return Object.freeze({
    ...
    scopes: Object.freeze([...new Set(scopes)].toSorted()),
  });
}
...
export async function routeArtifactReview(params: {
  ...
  try {
    ...
    if (item.kind !== "artifact_review" || !params.adapter) {
      return fail();
    }
    exact(
      item,
    ...
    if (params.method === "request") {
      ...
      if (item.decision !== "accept_artifact" && item.decision !== "reject_artifact") {
        return fail();
      }
      const resolution: ArtifactReviewResolution = Object.freeze({
        ...base,
        id: id(item.id),
        decision: item.decision,
      });
      command = resolution;
      returned = await params.adapter.resolve(resolution, trustedActor);
    }
    params.authorize?.();
    artifactReviewActorFromClient(params.client);
