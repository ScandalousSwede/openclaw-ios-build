...
describe("sendIosEvidenceNotification", () => {
  it.each(["direct", "relay"] as const)(
    ...
    async (transport) => {
      ...
      expect(f.options.authorizeSend).toHaveBeenCalledTimes(1);
      expect(f.deps.getPairedDevice).toHaveBeenCalledTimes(2);
      if (transport === "relay") {
        ...
        const relay = f.relayRequest.mock.calls[0][0];
        ...
        expect(f.directRequest.mock.calls[0][0].bearerToken.split(".")).toHaveLength(3);
      }
    },
  );
  ...
  it("accepts maximum scalar-count operation and event IDs without truncation", async () => {
    ...
    expect(f.directRequest.mock.calls[0][0].payload).toEqual({
      ...expectedPayload,
  ...
  it("omits the optional digest and drops arbitrary caller payload fields", async () => {
    ...
    expect(f.directRequest.mock.calls[0][0].payload).toEqual(expectedPayload);
  });
  ...
  it.each(["removed", "node-only", "revoked", "no-read", "not-ios", "no-token"])(
    ...
    async (state) => {
      ...
      if (state === "revoked") {
        paired.tokens!.operator.revokedAtMs = 2;
      }
      if (state === "no-read") {
        paired.tokens!.operator.scopes = ["operator.approvals"];
      }
      if (state === "not-ios") {
  ...
  it("accepts iPadOS with current inherited operator.admin read scope", async () => {
    ...
    paired.tokens!.operator.scopes = ["operator.admin"];
    f.setPairing(paired);
