...
it("does not leak invocation scope", () => {
  ...
  expect(getExplicitProviderRuntimeScope()).toBeUndefined();
});
...
it("resolves configured Anthropic alias before global metadata discovery", () => {
  ...
  const manifestPlugins = [
    {
      modelIdNormalization: {
        providers: { anthropic: { aliases: { sonnet: "claude-sonnet-4-20250514" } } },
      },
    },
  ];
  withExplicitProviderRuntimeScope({ config: cfg, provider, manifestPlugins }, (admitted) => {
    ...
    expect(selected?.modelId).toBe("claude-sonnet-4-20250514");
  });
});
