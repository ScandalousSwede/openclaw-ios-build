...
function immutableCopy<T>(value: T, seen = new WeakMap<object, unknown>()): T {
  ...
  if (prior) {
    // SAFETY: This map contains only a clone previously created from the exact same object.
    return prior as T;
  }
  if (
  ...
  // SAFETY: Every enumerable property is recursively copied without changing its value type.
