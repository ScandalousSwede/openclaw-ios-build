...
vi.mock("./facade-loader.js", async (importOriginal) => ({
  ...(await importOriginal<typeof import("./facade-loader.js")>()),
  loadBundledPluginPublicSurfaceModule: async ({
    dirName,
    artifactBasename,
  }: {
    dirName: string;
    ...
    expect(artifactBasename).toBe("api");
    if (dirName === "google") {
      return { buildGoogleProvider: owners.google };
    }
    if (dirName === "openai") {
      return { buildOpenAIProvider: owners.openai };
    }
    if (dirName === "anthropic") {
      return { buildAnthropicProvider: owners.anthropic };
    }
    throw new Error("Unreviewed public artifact");
  },
}));
