...
function lifecycle(state: HandoffReceipt["state"]) {
  switch (state) {
    case "queued":
      return "Queued for receiver";
    case "delivered":
      return "Delivered to receiver";
    case "acknowledged":
      return "Acknowledged by receiver";
  }
}
...
export class HandoffView extends LitElement {
  ...
  private updateGateway(snapshot: ApplicationGatewaySnapshot) {
    ...
    const ready = Boolean(snapshot.connected && snapshot.client && allowed);
    if (changed || ready !== this.ready) {
      this.generation++;
      this.busy = false;
      this.ready = ready;
  ...
  private async load(older = false) {
    const { client, connected, hello } = this.context.gateway.snapshot;
    if (
      !this.isConnected ||
      !client ||
      !connected ||
    ...
    if (older && (!before || this.error)) return;
    ...
    try {
      const page = parseHandoffPage(
        await client.request<unknown>("argus.handoffs.list", {
          limit: 5,
      ...
      if (page.items.length > 5) throw new Error("Invalid handoff page");
