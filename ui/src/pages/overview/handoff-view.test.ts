...
it("retires cached handoff bindings when read scope is revoked while disconnected", async () => {
  const { element, request, update } = await mount();
  await vi.waitFor(() => expect(element.textContent).toContain(receipt.summary));
  update(false);
  await element.updateComplete;
  expect(element.textContent).toContain(receipt.summary);
  update(false, false, []);
  await element.updateComplete;
  expect(element.textContent).not.toContain(receipt.summary);
  expect(element.textContent).not.toContain(receipt.binding.workspace);
